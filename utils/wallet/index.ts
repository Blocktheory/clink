...
export class Wallet {
  ...
  createAddressAndKeyUsingMnemonic = (mnemonic: string) => {
    const account = this.HDWallet.createWithMnemonic(mnemonic, "");
    const address = account.getAddressForCoin(this.CoinType.ethereum);
    const key = account.getKey(this.CoinType.ethereum, "m/44'/60'/0'/0/0");
    const keyHex = this.bufferToHex(key.data());
  ...
  getKeyFromPayLink = (url: string) => {
    ...
    try {
      ...
      return this.bufferToHex(
        account.getKeyForCoin(this.CoinType.ethereum).data()
      );
    } catch {
      return "";
    }
