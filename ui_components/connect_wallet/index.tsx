...
interface IConnectWallet extends THandleStep {
  signIn: () => Promise<void>;
  loader?: boolean;
  zerodevSignIn: () => Promise<void>;
}

export default function ConnectWallet(props: IConnectWallet) {
  const { signIn, loader, zerodevSignIn } = props;
  return (
    <>
      <div className="w-full relative lg:pt-10">
        <div className="w-full text-center p-2 relative">
          ...
          {loader ? (
            ...
            <div className="flex flex-col gap-3 justify-center items-center w-[80%] md:w-[60%] lg:w-[360px] mx-auto rounded-lg mt-10">
              <button
              ...
              <button className="bg-white rounded-lg px-4 py-2" onClick={zerodevSignIn}>Zerodev</button>
              <button className="bg-white rounded-lg px-4 py-2">disconnect</button>
            </div>
          )}
        </div>
      </div>
