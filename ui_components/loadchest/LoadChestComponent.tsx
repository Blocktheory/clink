...
export const LoadChestComponent: FC<ILoadChestComponent> = (props) => {
  ...
  const handleInitWallet = async () => {
    ...
    safeAccountAbs.setRelayPack(relayPack);
      ...
      try {
        if (loggedInVia === LOGGED_IN.GOOGLE) {
          if (isRelayInitiated.current) {
            ...
            const gelatoTaskId = await safeAccountAbstraction?.current?.relayTransaction(
              [safeTransactionData],
              options
            );
    ...
    <div className="mx-auto relative max-w-[400px]">
      {!transactionLoading ? (
          ...
          {!showActivity ? (
            <>
              <div className="rounded-lg border border-white/40 bg-white/5 ">
                <div className="flex items-center justify-between py-2 px-4">
                    ...
                    <div className="flex items-start gap-3 my-2">
                      ...
                      {toggle ? (
                        loading || loader ? (
                          ...
                          <div>
                            <p className="text-white/80 text-[24px] font-semibold leading-10 mb-2">{price}</p>
                            <p className="text-white/30 text-[12px] leading-[14px]">{tokenValue} ETH</p>
                          </div>
                        )
                      ) : loading ? (
                        <div className="w-full h-full">
                        ...
                        <div>
                          <p className="text-white/80 text-[24px] font-semibold leading-10 mb-2">~ {tokenValue} ETH</p>
                          <p className="text-white/30 text-[12px] leading-[14px]">{price}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Image
                      src={!loading && !loader ? icons.ethLogo : icons.loadAvatar}
                      className="w-8 h-8"
                      alt="transferIcon"
                    />
                    {!loading && !loader ? (
                ...
                <div className="relative rounded-lg border bg-white/5 border-gray-500  h-auto  p-4">
                  <div className="flex items-center justify-center">
                    <div>
                      ...
                      {Number(inputValue) > 0 && (
                        <p className="text-white/30 text-[12px] leading-[14px] text-center">~ {inputValue} ETH</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="relative mt-10">
                <div className={`${!btnDisable && value ? "opacity-100" : "opacity-50"} flex gap-2 justify-between`}>
                  <PrimaryBtn
        ...
        <div className="w-[full] max-w-[600px] h-full relative flex flex-col text-center items-center gap-5 mx-auto mt-20">
          <ReactTyped className="text-white text-[24px]" strings={[chestLoadingText]} typeSpeed={40} loop={true} />
