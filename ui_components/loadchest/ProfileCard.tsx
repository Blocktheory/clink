  ...
  return (
    <>
      <div className="w-full h-auto bg-[#7356C6] rounded-lg mb-4 profileBackgroundImage flex-col justify-center items-center text-center pb-2">
        <div className="pt-2">
          <Image src={address ? `https://effigy.im/a/${address}.png` : icons.loadAvatar} alt="profile image" width={50} height={50} className="w-12 h-12 rounded-full mx-auto border-white/50 border" />
        </div>
        <p className="text-sm mx-auto pt-1 text-white/80">{loggedInVia === LOGGED_IN.LENS ? `${handle}` : "My Smart Account"}</p>
        {transactionLoading ? <div className="w-20 h-3 my-2 animate-pulse bg-white/10 rounded-lg mx-auto"></div> : <p className="text-sm text-white pb-2">{`${trimAddress(address)}`}</p>}

        <div className="flex justify-around w-[100px] mx-auto pb-1">
          <Image src={icons.copyIconWhite} alt="copy address" className="w-5 cursor-pointer opacity-80 hover:opacity-100" onClick={copyToClipBoard} />
          <Image
            src={icons.qrWhite}
            alt="show qr code"
          ...
          />
          <Link href={`https://goerli.basescan.org/address/${address}`} target="_blank">
        ...
        <Link href={"https://www.biconomy.io/"} target="_blank">
          ...
          <Image src={icons.bicoLogo} alt="safe logo" className="w-20 inline-block" />
        </Link>
